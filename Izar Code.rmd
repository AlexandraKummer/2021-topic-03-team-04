...
exp_maxima_logic_max <- maxima_logic_func(exp_maxima_vec,0.6) #using maxima loc func on the tesing vector
...
planning implementation of the maxima detection function
apply to row 1 of RNAse and row 1 of Ctrl
iffy--Ctrl_row1 [maxima_loc_RNAse]/  RNAse_row1[maxima_loc_RNAse] same for ctrl
iffy--if ratio is > 1 increase , if < 1 decrease etc 

```{r}
'IMPLEMENTATION OF THE MAXIMA LOC FUNCTION FOR FACTOR DIF OF Y VAL IN MAXIMA LOC IN CTRL & RNASE'


delta_Y_fac_func <- function(x,y,threshhold){
  #factor_df <-data.frame(numeric())
  factor_df <- data.frame(matrix(ncol=1))
  colnames(factor_df)<-c("RNASe_factor")
   i <- 1
  while (i < dim(x)[1]){
    temp_logic <- maxima_logic_func(as.vector(x[i,]),threshhold)
    temp_max_loc_vec <- which(temp_logic)
    temp_factor_df <-unname(t(x[i,][temp_max_loc_vec]/y[i,][temp_max_loc_vec]))
    row.names(temp_factor_df,paste(row.names(x)[i],"frac",i:length(temp_max_loc_vec),sep = "_"))
...
"problen in rbind"# rbind takes 2 dfs and merges by rows, need ncol=ncol
if (ncol(factor_df)==ncol(temp_factor_df)){
  
    factor_df <-rbind(factor_df,temp_factor_df, make.names(c(paste(row.names(x)[i],"frac",i:length(temp_max_loc_vec),sep = "_"))))
}
