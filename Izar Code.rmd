...
median_dY_fac <- delta_Y_fac_func(RNAse_median_df,Ctrl_median_df,0.2)
median_dY_fac[-1,]

mean_dY_fac <- delta_Y_fac_func(RNAse_mean_df,Ctrl_mean_df,0.2)
mean_dY_fac[-1,]

lof_dY_fac <- delta_Y_fac_func(RNAse_lof_df,Ctrl_lof_df,0.2)
lof_dY_fac[-1,]
...
```{r}
library(ggplot2)
median_comp_gg_df <-as.data.frame(cbind(median_dY_fac[,1],Ctrl_median_one[,1]))
ggplot(median_comp_gg_df, aes(x=1:6833, y=median_comp_gg_df[,1]))+
  geom_point()+
  ggtitle("Shift factors for Ctrl peaks vs RNAse")+
  ylab("RNAse")+
  xlab("Ctrl")



hist(as.vector(median_comp_gg_df[,1]),)
